 ...
 * 
 * 任务 100200 = 是否能挑战扎昆
 * 任务 100201 = Collecting Gold Teeth <- indicates it's been started
 * 任务 100203 = Collecting Gold Teeth <- indicates it's finished
...
function action(mode, type, selection) {
    ...
    if (mode == 1) {
        ...
        if (status == 3) {
            cm.sendNext("看来不想这样啊？如果想明白了可以来找我！");
            cm.dispose();
        }
    ...
    if (status == 0) {
        ...
        if (teethmode == 1) {
            ...
            if (cm.haveItem(4000082, 30)) { // take away items, give eyes of fire, complete quest
                if (cm.canHold(4001017)) {
                    ...
                    cm.sendNext("冶炼好了。 看到左边的门了吗？它就是通往扎昆祭台的门。 不过你需要 #b#t4001017##k 才能进入里面。让我看看有多少人能进入到那个恐怖的地方？");
        ...
        if (selection == 0) { //ZPQ
            ...
            if (teethmode == 2 && cm.haveItem(4031061) && cm.haveItem(4031062)) {
                ...
                cm.sendYesNo("好吧， 你已经完成了早期的阶段。  现在， 努力一点我可以帮你得到进入扎昆祭台所需要的 火焰的眼。 但是， 我的牙齿最近有点疼。  你见过一个牙医在冒险岛世界的故事吗？  哦，我听说僵尸们有几颗金牙。我需要你找到 #b30 个僵尸丢失的金齿#k 。这样我就可以自己制造一些假牙。然后我可以帮你拿到你想要的物品\r\n任务要求：\r\n#i4000082##b x 30 个");
            } else {
                cm.sendNext("请完成上一阶段的任务再来挑战此阶段。");
                cm.dispose();
            }
            ...
            if (cm.getQuestStatus(100200) == 2) {
                ...
                cm.sendYesNo("你想收买我？哈哈，可以啊！但你必须给我 #e50,000,000#n 金币，我就可以让你直接跳过任务。");
                status = 3;
            }
        }
    } else if (status == 2) {
        if (stage == 1) {
            ...
            if (teethmode == 2) {
                if (cm.haveItem(4031061, 1) && cm.haveItem(4031062, 1) && cm.haveItem(4000082, 30)) { // take away items, give eyes of fire, complete quest
                    ...
                    cm.sendNext("我不认为你带来了30个 僵尸丢失的金牙呢……。请快点找来，我就会给你需要的东西。");
                    cm.dispose();
                }
            } else {
        ...
        if (cm.getPlayer().getMeso() < 5000000) {
            cm.sendNext("你好像没有足够的金币来支付，请检查一下再来。");
