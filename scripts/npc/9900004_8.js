    ...
    if (mode == -1) {
        ...
        if (status == 0) {
			...
			if(cm.getPlayer().getCSPoints(1) >= 3000){
				...
				cm.xlkc(7);
            cm.sendOk("购买成功！");
			cm.worldMessage(6,"玩家：["+cm.getName()+"]购买了天成冒险岛3000点卷项链扩充系统！");
            cm.dispose();
			}else{
            cm.sendOk("点卷不足无法购买！");
            cm.dispose();
			...
			if(cm.getPlayer().getCSPoints(1) >= 5000){
				...
				cm.xlkc(15);
            cm.sendOk("购买成功！");
			cm.worldMessage(6,"玩家：["+cm.getName()+"]购买了天成冒险岛5000点卷项链扩充系统！");
            cm.dispose();
			}else{
            cm.sendOk("点卷不足无法购买！");
            cm.dispose();
			...
			if(cm.getPlayer().getCSPoints(1) >= 8000){
				...
				cm.xlkc(30);
            cm.sendOk("购买成功！");
			cm.worldMessage(6,"玩家：["+cm.getName()+"]购买了天成冒险岛8000点卷项链扩充系统！");
            cm.dispose();
			}else{
            cm.sendOk("点卷不足无法购买！");
            cm.dispose();
			...
			if(cm.getPlayer().getCSPoints(1) >= 20000){
				...
				cm.xlkc(90);
            cm.sendOk("购买成功！");
			cm.worldMessage(6,"玩家：["+cm.getName()+"]购买了天成冒险岛20000点卷项链扩充系统！");
            cm.dispose();
			}else{
            cm.sendOk("点卷不足无法购买！");
            cm.dispose();
