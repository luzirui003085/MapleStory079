function enter(pi) {
    ...
    if (em != null && pi.getPlayer().getPosition().y <= -1240) {
        
        pi.warpS(pi.getMapId(), (pi.getPortal().getId() <= 4 ? (pi.isGMS() ? 13 : 0) : (pi.getPortal().getId() - 4)));
        pi.playerMessage( - 1, "Incorrect combination.");
	}else if (em != null && em.getProperty("stage6_" + (((pi.getMapId() % 10) | 0) - 1) + "_" + (pi.getPortal().getName().substring(2, 3)) + "_" + (pi.getPortal().getName().substring(3, 4)) + "").equals("1")) {
